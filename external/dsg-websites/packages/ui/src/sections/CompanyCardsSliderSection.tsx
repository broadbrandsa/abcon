...
export interface CompanyCardsSliderSectionProps {
...
export function CompanyCardsSliderSection({
    sectionHeading,
    sectionDescription,
    cards,
    tone = "light",
    className,
}: CompanyCardsSliderSectionProps) {
            ...
            <Container>
                <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
                    <div className="max-w-2xl">
                        {sectionHeading ? (
                            <Heading level={2} className={cn("mb-4", isDark ? "text-white" : "")}>
                                {sectionHeading}
                            </Heading>
                        ) : null}
                        {sectionDescription
                            ? typeof sectionDescription === "string"
                                ? (
                                    <Text className={cn("text-lg", isDark ? "text-white/80" : "text-muted-foreground")}>
                                        {sectionDescription}
                                    </Text>
                                )
                                : sectionDescription
                            ...
                            >
                                ...
                                {card.logoSrc ? (
                                    <div className="absolute left-6 top-6 z-10">
                                        <img
                                            src={card.logoSrc}
                                            alt={`${card.headline} logo`}
                                            className={card.logoClassName || "h-8 w-auto object-contain brightness-0 invert"}
                                        />
                                    </div>
                                ) : null}
                                ...
                                {card.href ? (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="absolute inset-0 z-20 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-[20px]"
                                    >
                                        <span className="sr-only">Visit {card.headline}</span>
                                    </a>
                                ) : null}
                            </div>
                        ))}
